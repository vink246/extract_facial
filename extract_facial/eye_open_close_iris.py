...
def get_eye_open_close_iris(
    eye_frame: np.ndarray,
    eyedecp1: Optional[float] = 45,
    eyedecp2: Optional[float]=25
):
    ...
    if eye_frame is not None:
        ...
        gray_eye_frame: np.ndarray = cv2.GaussianBlur(gray_eye_frame, (5, 5), cv2.BORDER_DEFAULT)

        # detect circles to find iris
        circlesle: np.ndarray = cv2.HoughCircles(
            gray_eye_frame,
            cv2.HOUGH_GRADIENT,
            1,
            100,
